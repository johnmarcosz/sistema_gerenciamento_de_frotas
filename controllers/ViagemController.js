    ...
    static async editarViagem(req, res) {
        ...
        Viagem.findOne({ where: { id: id }, raw: true })
            .then(async (viagem) => {
                ...
                motoristas.forEach(function(viagem_motorista){
                    idsMotoristas.push(viagem_motorista.id)
                })
 
                motoristas = []
 
                idsMotoristas.forEach(async function(idMotoristas){
 
                    var motorista = await Motorista.findOne({
                        where: {
                           id: idMotoristas
                        },
                        raw: true
                ...
                viagem.dataPartida= viagem.dataPartida.toLocaleDateString('en-CA')
                viagem.dataRetorno= viagem.dataRetorno.toLocaleDateString('en-CA')
